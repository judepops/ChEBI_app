    ...
    if st.session_state.active_page == 'About':
        ...
        if submit_button and id_list:
            ...
            for compound_id in query_ids:
                compound_id = compound_id.strip()  # Ensure no leading/trailing whitespace
                if compound_id:  # Check if the compound_id is not empty
                    results = search(compound_id)
                    if results:
                        # Only process the first result for each input
                        result = results[0]  # Taking the first result from the list
